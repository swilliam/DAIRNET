...
and the join
\begin{code}
grp :: (Eq act) =>Combinator act
grp ParseInfo {category=b,lexical=l2} ParseInfo {category=a, lexical=l1} 
        | a==b  = Just (ParseInfo {category=b, lexical=head_lex l1 l2 a b,combinator="|"})
          where head_lex l1 l2 a b = l1 ++ "+" ++ l2
grp _ _ = Nothing
\end{code}
\subsection{Lexicon}
\label{sec:lexicon}
...
renderParseTree t = pad 1.1 . centerXY
       $ renderTree
              (\n -> ((alignedText 0.5 0 (show (category n))) === strutY 1
                  === (alignedText 0.5 0 ((combinator n)++" - "++(lexical n)))) <> roundedRect 10 3 0.3 # fc white)
              (~~)
